...
export class ItemDetail {
    ...
    constructor(init?: ItemMasterData, paymentMethodCode: PaymentMethodCode = 'PayNow') {
        if (init) {
            ...
            this.ouomId = init.ougp?.baseUom || 0;
...
export class PurchaseOrder {
    ...
    toPayload(bonus: number = 0, bonusAmount: number = 0, total: number = 0, totalBeforeVatnumber: number = 0, vatAmount: number = 0): PurchaseOrder {
        ...
        // ⚠️ FIX: Clone address trước khi xóa field, tránh mutate _customer gốc trong store
        ...
        _payload.itemDetail = _payload.itemDetail.map((item) => { 
            item.priceAfterDist = 0; // [priceAfterDist] Giảm giá trên giá gốc
            item.distcountAmount = 0;
            item.vatAmount = 0; // [vatAmount] Thuế trên giá sau giảm
            item.lineTotal = 0;
            item.numInSale = 0; // [numInSale] Chả biết trường này để làm gì :D?, chắc là tổng sản lượng
