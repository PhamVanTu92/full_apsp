<template>
    <div>
        <DataTable v-model:selection="selection" :value="poStore.model.itemDetail" resizableColumns
            columnResizeMode="fit" showGridlines tableStyle="min-width: 50rem" scrollable>
            ...
            <Column :header="t('body.OrderList.quantity_column')" class="w-10rem">
                <template #body="{ data }">
                    <InputNumber v-model="data.quantity" :min="0.1" showButtons pt:input:root:class="w-10rem" />
                </template>
            </Column>
            <Column :header="t('PromotionalItems.PointConversion.table_header_temporary_points')" class="w-10rem">
                <template #body="{ data }">
                    <div class="text-right"> {{ (data.exchangePoint * data.quantity) || '--' }}</div>
                </template>
            </Column>
            <ColumnGroup type="footer">
                <Row>
                        ...
                        <template #footer>
                            <div class="flex justify-content-between">
                                <div class="flex gap-2">
                                    <ProductSelector v-if="poStore.model.cardId" :andPointApi="props.andPointApi"
                                        :label="t('body.PurchaseRequestList.find_product_button')"
                                        @confirm="onSelectProduct" :customer="poStore.model._customer" icon=""
                                        :activeRowPoint="true" />
                                    <template v-if="selection.length">
                                        <div class="flex justify-content-end gap-2">
                                            <Divider layout="vertical" class="mx-1"></Divider>
                                            <Button @click="onClickRemoveRow" size="small"
                                                :label="t('body.OrderList.delete')" severity="danger" />
                                        </div>
                                    </template>
                                </div>
                            </div>
                    ...
                    <Column :footer="t('body.home.total')" class="text-right" />
                    <Column :footer="fnum(totalQuantity || 0)" class="text-right" />
                    <Column :footer="fnum(totalPoints || 0)" class="text-right" />
                </Row>
            </ColumnGroup>
...
const totalQuantity = computed(() => {
    return poStore.model.itemDetail.reduce(
...
const totalPoints = computed(() => {
    return poStore.model.itemDetail.reduce(
        (sum, item) => item.exchangePoint * item.quantity + sum,
        0
    );
});
