...
export class PurchaseOrder {
    ...
    toPayload(bonus: number = 0, bonusAmount: number = 0, total: number = 0, totalBeforeVatnumber: number = 0, vatAmount: number = 0): PurchaseOrder {
        ...
        let taxRate = 0;
        let discountRate = 0;
        let priceAfterDiscountRate = 0;
        _payload.itemDetail = _payload.itemDetail.map((item) => {
            taxRate = (item.vat ?? 0) / 100;
            discountRate = (item.discount ?? 0) / 100;
            priceAfterDiscountRate = (item.discountType === 'P' ? item.price * discountRate : discountRate) || 0;
            item.priceAfterDist = item.price - priceAfterDiscountRate; // [priceAfterDist] Giảm giá trên giá gốc
            item.distcountAmount = priceAfterDiscountRate * item.quantity;
