...
import { CheckPricePayload, PromotionInCheckPricePayload, PromotionPayload } from '../script';
...
import { fetchPromotion, fetchCheckPrice } from '../script';
...
export const usePoStore = defineStore('PurchaseStore', () => {
    ...
    function setPromtionToShow(promotionOrderLine: PromotionOrderLine[]) {
        ...
        for (const item of promotionOrderLine) {
            ...
            // Gián số lượng sản phẩm được tặng vào trường _promotionQuanlity
            for (let item123 of model.value?.itemDetail) {
                const spTang = spTangArray.find((sp) => sp.itemId == item123.itemId);
                if (spTang) {
                    item123._promotionQuanlity = spTang.quantityAdd || 0; // Số lượng sản phẩm được tặng
                    // item._volumn = spTang.volumn || 0; // Số lượng quy đổi
                    item123._isPromotion = true; // Đánh dấu là sản phẩm khuyến mãi
                }
            }
    ...
    watch(
        () => JSON.stringify([model.value.itemDetail, orderSummary.value]),
        (value) => {
