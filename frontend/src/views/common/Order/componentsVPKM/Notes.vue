<template>
    <div class="">
        ...
        <div
            ...
            v-if="odStore.order?.status !== 'DHT' && !props.isClient"
        >
            <template v-if="editMode">
                <Button severity="secondary" :label="t('body.status.HUY2')" @click="onClickCancel"/>
                <Button
                    :loading="loadingNote"
                    :label="t('client.save')" @click="onClickSaveNote"
                />
            </template>
...
const onClickSaveNote = async () => {
    ...
    try {
        ...
        await API.patch(`PurchaseOrder/${odStore.order?.id}`, body as any);
        ...
        toast.add({
            ...
            detail: t('Notification.save_error_note'),
            life: 3000,
        });
    } finally {
