...
export type PaymentMethodCode = 'PayNow' | 'PayCredit' | 'PayGuarantee' | 'PayVisa';
export class ItemDetail {
    ...
    constructor(init?: ItemMasterData, paymentMethodCode: PaymentMethodCode = 'PayNow', quantity: number = 1) {
        if (init) {   
...
export class Customer {
    ...
    getCrD1Default(type: 'S' | 'B') {
        var _default = this.crD1.find((item) => item.type === type) || null;
        // var _default = this.crD1.find((item) => item.type === type && item.default === 'Y') || null; // lỗi khi tạo đơn hàng xóa địa chỉ của khách  
        return {
            default: _default,
