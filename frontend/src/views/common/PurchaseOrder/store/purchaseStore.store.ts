...
export const usePoStore = defineStore('PurchaseStore', () => {
    ...
    function setPromtionToShow(promotionOrderLine: PromotionOrderLine[] = []) {
        ...
        for (const item of promotionOrderLine) {
            ...
            // viết logic khác của hàm trên
            for (let itemDetail of model.value.itemDetail) {
                if (spGiamGiaArray.length > 0) {
                    for (let spgg of spGiamGiaArray) {
                        if (itemDetail.itemId == spgg.itemId) {
                            itemDetail._lastDiscount = spgg.discount || 0;
                            itemDetail.discount = spgg.discount || 0;
                        }
                    }
                }
            }
            ...
            for (let _item of model.value?.itemDetail) {
                ...
                if (spTang) {
                    _item._promotionQuanlity = spTang.quantityAdd || 1; // Số lượng sản phẩm được tặng
                    _item._isPromotion = true; // Đánh dấu là sản phẩm khuyến mãi
                } else {
                    _item._promotionQuanlity = 0; // Số lượng sản phẩm được tặng
                }
                _item._volumn = _item.volumn || 0; // Gán sản lượng sản phẩm 
            }
            ...
            line._selection = Object.keys(choiceGroup)[0];
    ...
    async function fetchCheckPriceMethod() {
        ...
        try {
            const res: AxiosResponse<{ item: OrderSummary; items: any; items1: any }> = await fetchCheckPrice(checkPricePayload);
    ...
    async function fetchPromotionMethod(callback?: Function) {
        ...
        try {
            const res = await fetchPromotion(prmtPayload);
            const promotion = res.data?.['items'];
            const promotionOrderLine = promotion?.['promotionOrderLine'] || [];

            rawPromotionOrderLine.value = promotionOrderLine;
            rawPromotion.value = promotion;
            model.value.Promotions = promotion;
            if (promotionOrderLine.length) {
                for (const promotionLine of promotionOrderLine) {
                    for (const subLine of promotionLine.promotionOrderLineSub) {
                        const itemInDetail = model.value.itemDetail.find(item => item.itemId === subLine.itemId);
                        if (itemInDetail && subLine.discountType) {
                            itemInDetail.discountType = subLine.discountType;
                        }
                    }
                }
            }
            try {
                setPromtionToShow(promotionOrderLine);
            } catch (err) {
                console.error('Lỗi lờ mao: ', err);
            }
            response = res.data;
            return res;
        } catch {
    ...
    const clearPromotion = () => {
        ...
        promotionToShow.value = [];
        for (let itemDetail of model.value.itemDetail) {
            itemDetail._lastDiscount = 0;
            itemDetail._volumn = itemDetail.volumn || 0;
        }
    };

    // trigger sẽ được gọi nếu data trong mảng sản phẩm thay đổi
    const itemChangeTrigger = async () => {
        if (model.value.docType === '')
            await fetchPromotionMethod();

        await fetchCheckPriceMethod();
    };
