<template>
  ...
  <DataTable
    v-if="poStore.promotionToShow.length"
    v-model:expandedRows="expandedRows"
    :value="poStore.promotionToShow"
    :showGridlines="true"
    class="mb-3"
  >
    <template #header>
    ...
    <Column
      v-if="
        poStore.getPromtionToShow?.some(
          (item) => Object.keys(item._choiceGroup).length || item._giftGroup?.length
        )
      "
      expander
      class="w-4rem"
    ...
    <template #expansion="slotProps">
      ...
      >
        ...
        >
          <Column class="w-selection" selectionMode="single"></Column>
          <Column field="itemName" class="" header="Tên sản phẩm">
            <template #body="{ data, field }">
              <div class="collapse-row">
                <div
                  v-for="(item, idx) in slotProps.data._promotionItems[data]"
                  :key="idx"
                  class="flex"
                >
                  <div>
        ...
        >
          ...
          <Column
            ...
            header="Số lượng"
          ></Column>
          <Column field="packingName" class="w-15rem" header="Đơn vị tính"></Column>
          <Column field="itemGroup" class="w-15rem" header="Loại sản phẩm">
            <template #body="{ data, field }">
...
<script setup lang="ts">
  ...
  const dbcer = debounce(() => {
    expandedRows.value = poStore.promotionToShow.filter((item) => {
      return Object.keys(item._choiceGroup).length > 0 || item._giftGroup?.length;
    });
    // expandedRows.value = poStore.getPromtionToShow;
  }, 10);
