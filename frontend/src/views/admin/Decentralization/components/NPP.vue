<template>
    ...
    >
            ...
            <div class="flex justify-content-between m-0">
                ...
                <div class="flex gap-2">
                    ...
                    <router-link :to="{ name: 'add-customer' }">
                        <Button icon="pi pi-plus" label="Thêm mới"/>
                    </router-link>
                </div>
            </div>
        </template>
        <template #empty>
            <div class="py-5 my-5 text-center">Không có người dùng nào</div>
        </template>
        ...
        <Column field="action" class="w-3rem">
            <template #body="slotProps">
...
const fetchAllUsersApsp = async (text = "") => {
    try {
        loading.value = true;
...
const dbSearch = debounce(() => {
    dataTable.value.pageNpp = 0;
    fetchAllUsersNpp(keyword.value);
}, 500);
const fetchAllUsersNpp = async (key = "") => {
    try {
        loading.value = true;
        let query = `Account/getall?limit=${dataTable.value.sizeNpp}&skip=${dataTable.value.pageNpp}&userType=NPP&OrderBy=id desc`;
        if (key) {
            query += `&search=${key}`;
        }
        const {
