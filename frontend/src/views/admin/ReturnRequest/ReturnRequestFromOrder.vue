...
import { useOrderDetailStore } from '../../common/Order/store/orderDetail';
...
const submitReturn = async () => {
  ...
  const payload = {
    ...
    cardCode: odStore.order?.cardCode,
    cardName: odStore.order?.cardName,
    currency: odStore.order?.currency,
  ...
  (newOrder) => {
    ...
    if (newOrder && newOrder.itemDetail) {
      orderProducts.value = newOrder.itemDetail.map((item) => ({
        ...
        price: item.price,
        ouomId: item.uoMEntry || item.ouomId,
        uomCode: item.uoMName || item.uomCode,
        selected: false,
        returnQty: 0,
        reason: '',
        errors: {}
