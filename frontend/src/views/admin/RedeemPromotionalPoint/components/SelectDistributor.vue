<template>
  ...
  >
    <InputText
      readonly fluid
      :value="placeholder"
      :placeholder="t('body.promotion.applicable_object_placeholder')"
      :class="{
  ...
  <Dialog
    ...
    :header="t('body.promotion.applicable_object_section_title')"
    :closable="true"
    class="w-9 md:w-7 lg:w-6"
    modal
  >
    <TabView v-model:activeIndex="activeIndex" :pt:panelContainer:class="'p-0'">
      <TabPanel :header="t('Custom.customer')">
        <DataTable
          id="cusdata"
          :selectAll="false"
          v-model:selection="selection.users"
          dataKey="id"
        ...
        >
          ...
          <template #empty>
            <div
              class="flex justify-content-center align-items-center py-5"
              style="height: 52vh"
            >
      ...
      <TabPanel :header="t('client.scopeCustomerGroup')">
        <!-- datastore.groups -->
        <DataTable
          v-model:selection="selection.groups"
          :value="datastore.groups"
          dataKey="id"
          selectionMode="multiple"
          scrollable
          scrollHeight="52vh"
    ...
    <template #footer>
      <div class="flex justify-content-between w-full">
        <div class="flex gap-3 align-items-center">
          <Button @click="onClickUnSelected" text severity="danger" outlined
            >{{t('Custom.deselect')}}</Button
          >
          <!-- <div>
            {{ placeholder }}
          </div> -->
  ...
  >
    <DataTable
      :value="selection.users.length ? selection.users : selection.groups"
    ...
    >
      ...
      <Column
        :field="selection?.type == 'G' ? 'groupName' : 'cardName'"
        :header="t('body.promotion.name')"
      ></Column>
    ...
    <template #footer>
      <div class="flex justify-content-between w-full">
        <Button
          :label="t('Custom.chooseCustomerOrGroup')"
          outlined
