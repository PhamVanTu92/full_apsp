<template>
    <div>
        <div v-if="poStore.getCommittedToShow.length" class="card p-0 border-green-700 collapse-border">
            <div v-for="(item, i) in poStore.getCommittedToShow" :key="i" class="p-3">
                <div class="flex justify-content-between align-items-center border-bottom-1 border-dashed border-300 border-x-none border-top-none">
                    <div class="font-bold text-lg text-500">{{ item.timeLabel }} {{ item.timeLabel === 'Năm' ? `` : item.timeValue }}</div>
                    <Button @click="committedReachRef?.open()" icon="pi pi-eye" text/>
                </div>
                <template v-for="(subItem, j) in item.data" :key="j">
                    <div class="my-2">
                        <span class="font-bold mr-2">{{ subItem.industryName }}: </span>
                        <span>{{ fnum(subItem.currentValue + subItem.incurredValue) }} / {{ fnum(subItem.quotaValue) }}</span>
                    </div>
                    <PercentBar
                        :arguments="[
                            {
