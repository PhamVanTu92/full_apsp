  ...
  <div class="border-1 border-green-700 p-3 border-round bg-white">
    <template v-for="(item, i) in lists" :key="i">
      <div class="flex justify-content-between my-3">
        ...
        <span v-if="item.field != 'bonusAmount'">{{
          poStore.model.currency == "USD" ? fnum(item.value, 2) : fnum(item.value, 0)
        }}</span>
        <span v-else class="text-green-600">
          <InputNumber
            v-model="poStore.orderSummary.bonusAmount"
            :min="0"
            :maxFractionDigits="2"
            class="text-green-600 w-8rem"
          />
        </span>
      </div>
    ...
    <div class="mt-5">
      <div class="flex justify-content-between text-lg">
        ...
        <span class="font-bold">{{
          fnum(poStore.orderSummary.totalAfterVat - poStore.orderSummary.bonusAmount, 2)
        }}</span>
      </div>
      <div class="mb-3">
        <Button
    ...
    <Button
      ...
      :disabled="
        ...
        poStore.orderSummary.totalAfterVat - poStore.orderSummary.bonusAmount < 0
      "
      iconPos="right"
...
<script setup lang="ts">
  ...
  import { fnum, createPurchaseOrder, clearCartItem } from "../types/script";
