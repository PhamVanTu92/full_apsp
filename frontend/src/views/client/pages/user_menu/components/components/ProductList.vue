<template>
  ...
  >
    ...
    <Column
      field="price"
      :header="
        !poStore.model.isIncoterm
          ? t('body.OrderList.unit_price_column')
          : t('body.OrderList.unit_price_column_usd')
      "
    >
      <template #body="{ data, field }">
        <InputNumber
    ...
    <Column
      :header="
        !poStore.model.isIncoterm
          ? t('body.OrderList.final_unit_price_column')
          : t('body.OrderList.final_unit_price_column_usd')
      "
      class="text-right"
      ...
      <template #body="{ data, field }">
        <InputNumber
          ...
          @update:modelValue="
            (val) => {
              if (data['discountType'] === 'P') {
                data.discount = (1 - data[field] / data.price) * 100;
              } else {
                data.discount = data.price - val;
              }
            }
          "
        />
    ...
    <Column
      :header="
        !poStore.model.isIncoterm
          ? t('body.OrderList.subtotal_before_tax_column')
          : t('body.OrderList.subtotal_before_tax_column_usd')
      "
      class="text-right"
    >
      <template #body="{ data }">
    ...
    <Column
      ...
      :header="t('body.OrderList.payment_method_label')"
      v-if="props.disabledRow.methodPayment"
    >
      <template #body="{ data, field }">
        <Dropdown
          v-model="data[field]"
...
const changeDiscoutType = (data: ItemDetail) =>{
  data.discount = 0;
  data.priceAfterDist = data.price
}
...
watch(
  ...
  (newLength, oldLength) => {
    if (newLength > oldLength) {
      const addedItems = poStore.model.itemDetail.slice(oldLength);
      addedItems.forEach((item) => {
        item.priceAfterDist = item.price;
      });
    }
  }
);
