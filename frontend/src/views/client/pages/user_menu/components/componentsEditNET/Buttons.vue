<template>
    <div>
        ...
        <HinhThucThanhToanDialog ref="htttDialogRef" type="NET" />
...
const huyDonHang = async () => {
    try {
        await API.update(`PurchaseOrder/${odStore.order?.id}/change-status/HUY2`);
        toastService('success', t('body.systemSetting.success_label'), 'Đã cập nhật trạng thái đơn hàng');
        odStore.fetchStore();
    } catch (err) {
...
const giaoHang = async () => {
    try {
        ...
        await API.update(`PurchaseOrder/${odStore.order?.id}/change-status/DGHR`, body);
...
const daGiaoHang = async () => {
    try {
        await API.update(`PurchaseOrder/${odStore.order?.id}/change-status/DGHR`);
        toastService('success', t('body.systemSetting.success_label'), t('Custom.deliverySuccessful'));
        odStore.fetchStore();
    } catch (err) {
        toastService('error', t('Custom.error'), t('Custom.errorOccurred'));
    }
    ...
    return [
        {
            ...
            severity: 'danger',
            visible: isVisible(['TTN', 'DXL', 'CXN'], !props.isClient),
