    ...
    <div class="flex flex-column gap-3">
        <template v-for="(p, i) in promotions" :key="i">
            <div class="border-1 border-200 p-3 surface-100">
                ...
                <DataTable v-if="p.data.filter((item) => item.quantityAdd > 0).length"
                    :value="p.data.filter((item) => item.quantityAdd > 0)" show-gridlines scrollable
                    scroll-height="235px">
                    <Column header="#" class="w-3rem">
                        <template #body="{ index }">{{ index + 1 }}</template>
                    </Column>
                    <Column field="itemName" :header="t('client.productName')"></Column>
                    <Column field="packingName" :header="t('client.unit')" class="w-10rem"></Column>
