    ...
    <div class="card p-4 border-round-xl shadow-1 mb-4 bg-white">
      <div class="flex flex-column gap-2">
        ...
        <InputText
          ...
          class="w-full p-3 border-1 border-200 focus:border-primary"
          disabled
        />
      </div>
    </div>
...
const removeReturnSelected = () => {
  const selectedCodes = new Set((selectedReturnProducts.value ?? []).map((item) => item.itemCode));
...
const submitRequest = async () => {
  ...
  const payload = {
    docType: customerInfo.value?.user?.bpInfo?.docType || '',
    cardName: customerInfo.value?.user?.bpInfo?.cardName || customerInfo.value?.user?.fullName || '',
    cardCode: customerInfo.value?.user?.bpInfo?.cardCode || '',
    cardId: customerInfo.value?.user?.cardId || 0,
    refInvoiceCode: null,
...
onMounted(async () => {
  ...
  customerInfo.value = me;
  customerName.value = me?.user?.bpInfo?.cardName || me?.user?.fullName || '';
});
</script>
