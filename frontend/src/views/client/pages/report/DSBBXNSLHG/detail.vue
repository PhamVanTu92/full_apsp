        ...
        <div class="detail-grid">
            <section class="document-panel">
                ...
                <div class="document-preview" :class="{ 'has-preview': canPreviewFile }">
                    ...
                    <template v-else>
                        ...
                        <div class="preview-text">{{ t("client.minutes_content_placeholder") }}</div>
                        <button class="download-link" type="button" :disabled="!record.filePath" @click="openFile">
                            {{ t("client.download_to_view_detail") }}
                        </button>
                    </template>
                </div>
            </section>
            ...
            <aside class="side-panel">
                <section class="action-card">
                    <h4>{{ t("client.confirm_minutes_title") }}</h4>
                    <template v-if="record.canRespond">
                        <label for="feedbackNote">{{ t("client.feedback_note") }}</label>
                        <Textarea
                            id="feedbackNote"
                            v-model="feedbackNote"
                            rows="5"
                            class="feedback-input"
                            :placeholder="t('client.feedback_placeholder')"
                        />
                        <div class="action-row">
                            <Button
                                class="reject-button"
                                icon="pi pi-times-circle"
                                :label="t('client.reject')"
                                outlined
                                severity="danger"
                                :loading="rejecting"
                                @click="rejectMinutes"
                    ...
                    <div v-else-if="isConfirmed" class="confirmation-result approved-result">
                        <div class="confirmation-result-title">Bạn đã xác nhận biên bản này</div>
                        <div v-if="confirmedTime" class="confirmation-result-time">Vào lúc: {{ confirmedTime }}</div>
                    </div>
                    <div v-else-if="isRejected" class="confirmation-result rejected-result">
                        <div class="confirmation-result-title">Bạn đã từ chối biên bản này</div>
                        <div v-if="rejectedTime" class="confirmation-result-time">Vào lúc: {{ rejectedTime }}</div>
                    </div>
                </section>
...
const approvedHistory = computed(() =>
    histories.value.find((item) => item.iconClass === "approve-history-icon")
);

const rejectedHistory = computed(() =>
    histories.value.find((item) => item.iconClass === "reject-history-icon")
);

const isConfirmed = computed(() =>
    record.value.status === "2" ||
    !!record.value.approvedAt ||
    !!approvedHistory.value
);

const isRejected = computed(() =>
    record.value.status === "3" ||
    !!record.value.rejectedAt ||
    !!rejectedHistory.value
);

const confirmedTime = computed(() => record.value.approvedAt || approvedHistory.value?.time || "");

const rejectedTime = computed(() => record.value.rejectedAt || rejectedHistory.value?.time || "");
...
const approveMinutes = async () => {
    ...
    try {
        ...
        await approveConfirmation(record.value.id, feedbackNote.value || "");
        notify("success", "Thành công", "Đã xác nhận biên bản");
        await fetchDetail();
    } catch (error) {
        notify("error", "Lỗi", getErrorMessage(error, "Xác nhận biên bản thất bại"));
    } finally {
        approving.value = false;
    }
};

const rejectMinutes = async () => {
    if (!record.value.id) return;

    try {
        rejecting.value = true;
        await rejectConfirmation(record.value.id, feedbackNote.value || "");
        notify("success", "Thành công", "Đã từ chối biên bản");
        await fetchDetail();
    } catch (error) {
        notify("error", "Lỗi", getErrorMessage(error, "Từ chối biên bản thất bại"));
    } finally {
...
const goBack = () => {
    router.push({ name: "client-confirmation-minutes" });
};
...
.confirmation-result-title {
    font-weight: 800;
    line-height: 1.35;
}

.confirmation-result-time {
    font-size: 0.8rem;
    margin-top: 0.9rem;
}
