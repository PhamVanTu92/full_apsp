...
public class PurchaseOrderWorkFlow(IHttpContextAccessor httpContextAccessor, AppDbContext context)
    : BaseWorkFlowEngineService
{
    public override DocumentEnum DocumentType => DocumentEnum.PurchaseOrder;
    ...
    public override async Task<List<Models.Approval_V2.ApprovalSample>> IsApprovalWorkFlow(int docId)
    {
        ...
        var purchaseOrder = await context.ODOC.FirstOrDefaultAsync(x => x.Id == docId && x.ObjType == 22);
        ...
        var isExist = await context.ApprovalSamples
            ...
            .Where(x => x.IsActive
                ...
                && (
                    (x.IsOverdueDebt && purchaseOrder.LimitOverDue == true)
                    ||
                    (x.IsDebtLimit && purchaseOrder.LimitRequire == true)
                    && x.ApprovalSampleDocumentsLines.Any(line => line.Document == DocumentEnum.PurchaseOrder)
                ))
            .ToListAsync();
        return isExist;
    }
